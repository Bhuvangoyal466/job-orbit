...
const updateCandidateProfile = async (req, res, next) => {
  try {
    ...
    const allowedFields = [
      ...
      'preferredJobType', 'expectedSalary', 'preferredLocations'
    ];
...
const getDashboardStats = async (req, res, next) => {
  try {
    ...
    const stats = {
      ...
      hiredApplications: candidate.applications.filter(app => 
        app.status === 'hired'
      ).length,
      rejectedApplications: candidate.applications.filter(app => 
        app.status === 'rejected'
      ).length,
